...
import { ServiceLanding } from "../data/serviceLandingsData";
...
export const ServiceLandingPage: React.FC<ServiceLandingPageProps> = ({
  ...
  useEffect(() => {
    ...
    // Inject dedicated Service Schema.org JSON-LD
    ...
    return () => {
      // Clean up dynamic schema when leaving landing page
  ...
  const handleWhatsApp = () => {
    const text = encodeURIComponent(
      `Ciao Moockup Studio! Vorrei maggiori informazioni e un preventivo dedicato per il servizio "${landing.title}".`
    );
    window.open(`https://wa.me/${AGENCY_INFO.whatsapp}?text=${text}`, "_blank", "noopener,noreferrer");
  };

  const handleEmail = () => {
    const subject = encodeURIComponent(`Richiesta Preventivo per ${landing.title}`);
    const body = encodeURIComponent(
      `Ciao Moockup Studio,\n\nVorrei ricevere una proposta e maggiori dettagli per il servizio "${landing.title}".\n\nI miei recapiti:\nNome:\nAzienda:\nTelefono:`
    );
  ...
  return (
    <div className="min-h-screen bg-[#090D16] text-[#E2E8F0] selection:bg-fuchsia-600 selection:text-white">
      {/* Top Floating Navigation Bar for Landing */}
      <header className="sticky top-0 z-40 bg-[#090D16]/90 backdrop-blur-md border-b border-slate-800/80 py-3 px-4 sm:px-8">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button
              onClick={onBack}
              id="landing-back-home-btn"
      ...
      <section className="relative pt-12 pb-20 overflow-hidden border-b border-slate-800/80">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <div className="max-w-3xl mx-auto text-center space-y-6">
            ...
            <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-3">
              ...
              >
                ...
                <span>Audit Tecnico Gratuito</span>
              </button>
            </div>

            <p className="text-xs text-slate-500 flex items-center justify-center gap-2">
              <Clock className="w-3.5 h-3.5 text-emerald-400" />
              <span>Risposta immediata in meno di 2 ore lavorative • Zero impegno</span>
            </p>
          </div>
          ...
          <div className="mt-12 rounded-2xl overflow-hidden border border-slate-800 shadow-2xl relative max-w-4xl mx-auto w-full aspect-[16/9] sm:aspect-[21/9] min-h-[260px] bg-slate-950">
            ...
            <div className="absolute bottom-6 left-6 right-6 p-4 rounded-xl bg-black/60 backdrop-blur-md border border-white/10 text-xs sm:text-sm text-slate-200">
              <span className="font-bold text-pink-400">Target Prestazionale: </span>
              {landing.metricsTarget}
            </div>
          </div>
        </div>
      </section>

      {/* Why Choose Moockup for this Service */}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-14">
            <h2 className="font-display font-extrabold text-2xl sm:text-4xl text-white mb-3">
              Perché Scegliere Moockup Studio per {landing.title}
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {landing.whyChoosePoints.map((point, idx) => (
              <div 
                key={idx}
      ...
      <section className="py-20 border-b border-slate-800/80">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="p-8 sm:p-12 rounded-3xl bg-gradient-to-b from-slate-900 to-slate-950 border border-slate-800 space-y-8">
            <div>
              <div className="text-xs font-bold uppercase tracking-wider text-pink-400 mb-2">Cosa Riceverai</div>
              <h2 className="font-display font-bold text-2xl sm:text-3xl text-white">
                Tutto Incluso nel Nostro Servizio Chiavi in Mano
              </h2>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {landing.deliverables.map((item, idx) => (
            ...
            <div className="pt-6 border-t border-slate-800/80 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                <div className="font-bold text-white text-sm sm:text-base">Pronto a far decollare il tuo progetto?</div>
                <div className="text-xs text-slate-400">Nessun costo nascosto. Preventivo trasparente e bloccato.</div>
              </div>
              <button
                onClick={handleWhatsApp}
              ...
              >
                ...
                <span>Richiedi Subito Preventivo</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <section className="py-20 border-b border-slate-800/80 bg-[#0B0F1C]">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="font-display font-extrabold text-2xl sm:text-3xl text-white text-center mb-10">
              Domande Frequenti su {landing.title}
            </h2>
      ...
      {/* Bottom CTA & Direct Contact */}
      <section className="py-20 text-center relative">
        <div className="max-w-3xl mx-auto px-4 space-y-6">
          <Logo size="lg" showTagline={true} />
          <h2 className="font-display font-bold text-2xl sm:text-4xl text-white">
            Parliamo del Tuo Progetto Oggi Stesso
          </h2>
          <p className="text-slate-400 text-sm sm:text-base">
            Siamo a tua completa disposizione per analizzare la tua situazione attuale e formulare una strategia su misura per la tua attività.
          </p>
